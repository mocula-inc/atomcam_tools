...
起動直後にまだ一度もLANへ接続できていない場合（ルーターの起動がカメラより遅い場合など）は、
最大20分程度は再起動をせずに待ち、5分おきに再接続を試みます。一度接続できたあとで
切断された場合はより早く（2〜6分程度で）復旧を試みます。

##### - 異常時再起動

LANとの接続ができない状態が継続した場合、システムの再起動をします。
起動直後から一度も接続できていない場合は約20分、一度接続できたあとに切断した場合は
約6分で再起動します（再起動の回数に上限はありません）。

接続できない原因は `/media/mmc/netdiag.log` に記録されます（異常が起きたときだけ書き込まれ、
正常時は何も書き込まれません）。ログの `cause=` を見ることで原因を切り分けられます。

| cause | 意味 |
|---|---|
| `NO_WLAN_DEV` | WiFi ドライバが読み込まれていない |
| `NO_SUPPLICANT` | wpa_supplicant が起動していない |
| `SSID_UNCONFIGURED` | WiFi の設定（ペアリング）がされていない |
| `RADIO_DEAD` | 周囲の AP が1つも見えない（無線チップの異常の可能性） |
| `AP_NOT_FOUND` | 設定した SSID の電波が見つからない（ルーターが起動中、SSID変更、電波が届かない等） |
| `AUTH_FAILED` | パスワードが間違っている可能性 |
| `ASSOC_REJECTED` | ルーターに接続を拒否された（MACフィルタ、接続台数上限等） |
| `NO_DHCP_LEASE` | 接続はできたが IP アドレスが取得できない |
| `NO_DEFAULT_ROUTE` | IP は取得したがルーター（ゲートウェイ）情報がない |
| `GW_UNREACHABLE` | ルーターへの応答がない |
