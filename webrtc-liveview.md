...
カメラ単位ロックが `LOCK_TTL_MARGIN_SECONDS` の猶予いっぱいまで残留し、その間ブラウザは
...
ロックの TTL はセッション作成時に一度だけ、`SESSION_TTL_SECONDS(600秒) + LOCK_TTL_MARGIN_SECONDS(120秒)`
に固定される(以降は延長しない)。この固定長はカメラ側 `live_sessionMax` の現実的な最大値を
覆うように設定されているため、視聴が正常に続く限りロックが途中で失効することはない。
先行セッションが `DELETE`(ブラウザ)または `/end`(カメラ、下記参照)で明示的に終了すると
即座にロックが解放され、以後の作成が成功するようになる。ブラウザ・カメラの双方が応答不能に
なる異常系でも、ロックはセッション作成時刻から高々 720 秒後には必ず自然失効する(視聴時間に
応じて無限に延びることはない)。
