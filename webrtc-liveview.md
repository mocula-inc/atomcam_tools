...
> API の正本はバックエンドの OpenAPI 定義 (`mocula-backend/docs/api.yaml`) を参照。
> ブラウザ組み込みの手順は `mocula-backend/client/README.md` にまとめてある。
...
### `POST /api/v1/live-signal/{tenantKey}/{cameraKey}/end?session={id}`

カメラが backend からの `stop`/`gone` 指示を経ずに**自発的に**セッションを終える経路
（`live_sessionMax` 到達）で呼ぶ。呼ばないと backend 側セッションが `CLOSED` に遷移せず、
カメラ単位ロックが引き上げ後の TTL いっぱい（offer 取得から 750 秒）まで残留し、その間ブラウザは
image フォールバックへ倒れたまま何のフレームも受け取れなくなる
（`session_status`/`image_mode` が `stop` や 404/410 を受けて終わる経路では backend が既に
終了を把握しているため不要）。

**レスポンス**: `204`（セッションが既に存在しない/`CLOSED` 済みの場合も冪等に成功）
...
リクエストが来た場合の TOCTOU レースを閉じる)の二段構えで多重セッションを防ぐ。

ロックの TTL は **2 段階**で与えられる。

1. **セッション作成時 (offer 取得前)**: `180秒`（camera-sync 周期 60 秒 × 3）。
   カメラはまだ何も知らない状態なので、短い「offer 取得猶予」しか与えない。
2. **カメラが `GET /offer` を取得した時点**:
   `SESSION_TTL_SECONDS(600秒) + SESSION_TTL_GRACE_SECONDS(30秒) + LOCK_TTL_MARGIN_SECONDS(120秒)`
   へ一度だけ引き上げる(セッション本体の TTL 引き上げと同一トランザクション)。以降は延長しない。

セッション本体の TTL は `600 + 30 = 630秒`。**上限そのもの(600秒)と同じ時刻にしてはならない。**
セッションの読み取りは期限切れを除外するフィルタを通るため、両者が同時刻に交差すると必ず
フィルタが先に効き、上限判定 → `closeSession`(ロック即解放)に到達しない。その結果ロックが
マージンぶん残留し、上限で切られた視聴者が直後に再接続すると 409 になる(実機で再現・修正済み)。
上限での終了はあくまでアプリ層の判定が担い、TTL は「カメラもブラウザも二度と戻ってこない」
異常系のための安全網に徹させる。

この 2 段階化は「ブラウザが視聴を開始したがカメラがオフラインで一度も offer を取りに来ず、
ブラウザも `DELETE` を送らずに離脱した」ケースで、カメラが一度も動いていないのにロックだけが
実配信ぶん(10 分超)残り、その間ほかの視聴者が一切開始できなくなるのを防ぐためのもの。
ブラウザ SDK は answer 待ちを 120 秒 (`ANSWER_WAIT_TOTAL_MS`) で打ち切るため、猶予をこれ以上
長くしても「視聴者はとうに諦めているのにロックだけが残る」時間が延びるだけで益がない。
逆にこの猶予を SDK の待ち時間より短くしてもいけない(ブラウザがまだ待っている最中にセッションが
失効し、カメラが offer を取りに来ても 404 で弾かれる)。

SDK の 120 秒は実測に基づく。同期周期の待ち 0〜60 秒 + 開始検知から offer 取得まで 21〜25 秒 +
offer から answer 提出まで 1〜5 秒で、最悪およそ 90 秒。旧値の 65 秒では、同期直後に視聴を
開始すると必ずタイムアウトしていた。

> **注意**: この 60 秒は `mocula.sh` のメインループに固定で埋め込まれた camera-sync 周期であり、
> API が返す `checkInterval` とは**別物**。`checkInterval` はカメラ側では画像アップロードの
> 頻度にのみ使われ、同期間隔には影響しない（実機では `checkInterval=1800` でも
> camera-sync は 60 秒周期）。猶予の算出に `checkInterval` を使ってはならない。

引き上げ後の固定長はカメラ側 `live_sessionMax` の現実的な最大値を覆うように設定されているため、
視聴が正常に続く限りロックが途中で失効することはない。先行セッションが `DELETE`(ブラウザ)
または `/end`(カメラ、下記参照)で明示的に終了すると即座にロックが解放され、以後の作成が
成功するようになる。ブラウザ・カメラの双方が応答不能になる異常系でも、ロックは offer 取得時刻から
高々 750 秒後には必ず自然失効する(視聴時間に応じて無限に延びることはない)。
...
**フレームがまだない場合**: `503`(セッションは有効。ポーリング継続)  
**セッションが終了している場合**: `410`(上限到達・`DELETE`・TTL 失効)

`503` と `410` の区別は必須。両方を `503` にしていた頃は、10 分の上限でセッションが閉じても
ブラウザがそれを知る術がなく、静止画フォールバックのまま `503` を延々とポーリングして画面が
固まっていた(実機で確認)。同じ理由で `POST /ice-failed` も、終了済みセッションには `204` では
なく `410` を返す。SDK は `410`/`404` を「正常終了」と解釈し、静止画へ倒さずそのまま終了する。
