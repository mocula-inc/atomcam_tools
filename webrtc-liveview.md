...
カメラ単位ロックが引き上げ後の TTL いっぱい（offer 取得から 720 秒）まで残留し、その間ブラウザは
...
2. **カメラが `GET /offer` を取得した時点**: `SESSION_TTL_SECONDS(600秒) + LOCK_TTL_MARGIN_SECONDS(120秒)`
...
ブラウザ SDK は answer 待ちを 65 秒で打ち切るため、猶予をこれ以上長くしても
「視聴者はとうに諦めているのにロックだけが残る」時間が延びるだけで益がない。
...
高々 720 秒後には必ず自然失効する(視聴時間に応じて無限に延びることはない)。
