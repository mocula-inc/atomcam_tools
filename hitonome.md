...
ATOM Cam から定期的に JPEG 画像をキャプチャし、hitonome クラウド API と連携して S3 にアップロードするデーモンスクリプト。

- **スクリプトパス**: `/scripts/hitonome.sh`
- **設定ファイル**: `/media/mmc/mconfig`
- **ログファイル**: `/tmp/log/hitonome.log`
- **PID ファイル**: `/var/run/hitonome.pid`
- **起動スクリプト**: `/etc/init.d/S76hitonome`
...
[dev]
apiOrigin=https://qa1.mocula-dev.com  # 省略時は https://hitonome.cloud
```
...
```sh
/scripts/hitonome.sh on       # デーモン起動
/scripts/hitonome.sh off      # デーモン停止
/scripts/hitonome.sh restart  # 再起動
/scripts/hitonome.sh watchdog # プロセス死活監視（cron から毎分呼び出し）
```
...
| ログメッセージ | 原因 |
|--------------|------|
| `hitonome started (pid=...)` | デーモン起動 |
| `hitonome stopped` | `off` コマンドによる停止 |
| `watchdog: restarting hitonome` | watchdog によるプロセス再起動 |
...
iCamera_app（カメラ純正ファームウェア）が起動時に `/tmp/resolv.conf` を上書きするため、udhcpc が設定した DNS が失われる。`S76hitonome` の `start` 処理で `udhcpc` に `USR1 (RENEW)` シグナルを送ることで回避している。

```sh
# /etc/init.d/S76hitonome より
kill -USR1 $(pgrep udhcpc) > /dev/null 2>&1
sleep 1
/scripts/hitonome.sh on &
```
